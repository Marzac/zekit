...
\title{
    ...
    \vspace*{0.25cm}
    \small{Revision 1.0 EN - 27/08/2021} \\
    \vspace*{0.5cm}
...
I would personally like to thank the following persons for their contributions:
...
The \textbf{10k pots} are labeled \textbf{B103} at the bottom whereas the \textbf{100k pots} are labeled \textbf{B104}.
...
(*) H11L1 can be in black or white DIP (dual in-line package). They are both as great.

Before positioning the ICs, it is recommended to \textbf{carefully bend} their legs so they are made \textbf{vertical.} It will be easier to insert them in the sockets.
...
\begin{itemize}
    ...
    \item \textbf{VCF Q} switch \emph{TG2} to bottom
    \item \textbf{EXT IN} switch \emph{TG2} to top
\end{itemize}
...
The \textbf{ZeKit} has been designed to be more than a \emph{quickly assembled and affordable synth kit.} The design material is provided under \textbf{Creative Commons and GNU licenses} that allows you to \textbf{modify, extend and build upon} this project, as long as this \textbf{isn't for commercial purposes} (or contact us).

So let \textbf{your imagination be at the driving seat,} here are areas to explore:

\begin{itemize}
    \item \textbf{Software modification}\\ Firmware code is on GitHub
    \item \textbf{Hardware modding}\\ Test points are provided
    \item \textbf{Front panel design}\\ Template are on GitHub
...
Here is a list of the \textbf{common troubles} you may be facing with:
...
\begin{center}
    \includegraphics[scale=0.65]{assets/schema-vcf.png}
\end{center}
\vspace{0.25cm}
...
It alternates the MCU pin mode connected to the switch signal between input and open-drain modes.
